...
public class Maths {
    ...
    public static Matrix4f createViewMatrix(Camera camera) {
        Matrix4f matrix = new Matrix4f();
        matrix.setIdentity();
        matrix.rotate((float) Math.toRadians(Camera.getPitch()), new Vector3f(1, 0, 0), matrix, matrix);
        matrix.rotate((float) Math.toRadians(Camera.getYaw()), new Vector3f(0, 1, 0), matrix, matrix);
        //matrix.rotate((float) Math.toRadians(Camera.getYaw()), new Vector3f(0, 0, 1), matrix, matrix);
